...
<body><main><div class="card">
  ...
  <div class="meta">
    ...
    <div class="label">Playtest job</div><div><code>${escapeHtml(preview.externalJobId)}</code> revision ${preview.revision}</div>
  </div>
...
export function renderDelegatedConsentErrorPage(status: number): string {
  ...
  return renderErrorDocument(title, "Return to Playtest and create a new preview if needed.");
}
...
export function renderDelegatedConsentCsrfErrorPage(): string {
  return renderErrorDocument(
    ...
    "Return to the original Prism approval page and try again. If the request still cannot be verified, return to Playtest and create a new preview."
  );
}
...
function renderErrorDocument(title: string, message: string): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="referrer" content="no-referrer"><title>${escapeHtml(title)} · Prism</title></head><body><main><h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p></main></body></html>`;
}
